...
const botName = process.env.BOTNAME || "GHOST-XMD";
    ...
    try {
      await socket.sendMessage(botJid, {
          text: [
            ...
            '',
            '💳 Renew at *hosting.ghost.tech* to keep using the bot.',
            '',
            '_Ghost-Hosting_'
          ].join('\n')
        });
        ...
        try {
          await socket.sendMessage(botJid, {
            text: [
              ...
              '',
              'Your GHOST-XMD bot subscription has expired!',
              '',
              'This bot will be permanently deleted in 24 hours unless you renew.',
              '',
              'Renew now at: hosting.ghost.tech',
              '',
              '- Ghost Hosting'
            ].join('\n')
          });
