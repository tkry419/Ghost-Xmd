...
import toxicaiFeature from '../features/bmbai.js';
...
try {
    _toxicPict = fs.readFileSync(path.resolve(__dirname, '../bmbxmd.jpg'));
} catch (e) {
...
export default async (client, m, chatUpdate, store) => {
    ...
    try {
        ...
        if (settings?.toxicagent === true || settings?.toxicagent === 'true') {
            toxicaiFeature({ client, m, body, isDev, settings, fq: fakeQuoted, prefix: usedPrefix }).catch(e => console.log('❌ [BMBAI]:', e.message));
        }
