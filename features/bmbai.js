...
function buildSystemPrompt(lastRepo) {
    ...
    return `You are GHOSTAgent — a hyper-capable GitHub AI assistant that is perpetually exhausted and mildly offended by having to exist. You work exclusively for ghostxmd (GitHub username: ghostxmd).
...
export default async (context) => {
    ...
    async function uploadFile(owner, repo, filePath, content, message) {
        try {
            ...
            const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`, {
                ...
                body: JSON.stringify({ message: message || 'Upload via GHOSTAgent', content: encoded })
            });
    ...
    try {
        ...
        await client.sendMessage(m.chat, { text: boxWrap(finalReply, 'GHOSTAGENT') });
        if (imageUploadedUrl) {
